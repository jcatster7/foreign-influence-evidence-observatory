...
`audit_screening.ts` validates reviewer decisions in this pre-registration queue and currently reports `ready_for_final_study_count: false`: the fifth query is incomplete, all 7,986 records lack title decisions, and 1,599 calibration records lack a second reviewer. The final-count flag remains false by design even if this scoping queue is screened, because the registered search, supplemental sources, publication-to-dataset merges, and dual-coded extractions are separate release requirements. A fixture confirmed that a record advanced to full text also fails the scoping gate until two full-text decisions exist.
