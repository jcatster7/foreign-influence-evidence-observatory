...
Version 0.2, 2026-09-20. Prepared for an immutable GitHub preregistration release. The 2026-09-19/20 API acquisitions are exploratory, known before registration. Any changes after the release must be logged as deviations.
...
After the GitHub preregistration packet is published as an immutable release, run `search_openalex_registered.ts` with that release URL and the final UTC cutoff date. Before creating a run, it checks the public GitHub Releases API for an immutable release dated on or before the cutoff, verifies the attached packet SHA-256 against the local packet, and checks the current harvester, queue builder, and protocol against the packet manifest. It records the release timestamp, asset hash, and verification time in `RUN.json`. Each raw JSON response has a paired checkpoint containing its hash and cursor. A resumed run verifies its fixed query, URL, cursor, and raw-byte hash before accepting a checkpoint. An HTTP limit or safety cap leaves the run incomplete. Optional `OPENALEX_API_KEY` authenticates with a bearer header; the key must never enter URLs, checkpoints, or the repository. The run records whether keyless or environment-key access was used. The query, cutoff, selected fields, and screening rules do not change with authentication. The earlier `search_openalex_full.ts` and its derived page records are scoping material only; they did not retain the raw response bytes and cannot substitute for this preregistered acquisition.
