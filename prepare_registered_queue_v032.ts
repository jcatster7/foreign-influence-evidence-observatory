...
const run = JSON.parse(readFileSync(resolve(directory, 'RUN.json'), 'utf8')) as
  { registration_url: string; cutoff_utc_date: string; queries: string[]; status: string;
    registration_kind?: string; registration_verified_at_utc?: string;
    registration_published_at_utc?: string; registration_asset_sha256?: string };
const fixedQueries = [
...
if (run.status !== 'complete' || JSON.stringify(run.queries) !== JSON.stringify(fixedQueries) ||
    !/^https:\/\/github\.com\/jcatster7\/foreign-influence-evidence-observatory\/releases\/tag\/v[0-9][a-z0-9.-]*$/.test(run.registration_url) ||
    run.registration_kind !== 'github_immutable_release' || !run.registration_verified_at_utc ||
    !run.registration_published_at_utc || !/^[a-f0-9]{64}$/.test(run.registration_asset_sha256 ?? '') ||
    run.registration_published_at_utc.slice(0, 10) > run.cutoff_utc_date) {
  throw new Error('A complete five-query immutable GitHub-preregistered OpenAlex run is required');
}
const sha256 = (value: string) => createHash('sha256').update(value).digest('hex');
type Work = { id: string; display_name: string | null; doi: string | null; publication_year: number | null;
...
for (let qi = 1; qi <= 5; qi++) {
  ...
  while (cursor) {
    ...
    for (const work of response.results) {
      if (!work.id?.startsWith('https://openalex.org/W') ||
          (work.display_name !== null && typeof work.display_name !== 'string')) throw new Error(`Invalid work: ${base}`);
      if (!work.display_name) rawRowsMissingTitle++;
      ...
      if (prior) {
        ...
        if (!prior.title && work.display_name) prior.title = work.display_name;
        continue;
      }
...
for (const item of [...byOpenAlex.values()].sort((a, b) => a.openalex_ids[0].localeCompare(b.openalex_ids[0]))) {
  ...
  if (prior) {
    ...
    if (!prior.title && item.title) prior.title = item.title;
...
const summary = { status: 'registered_openalex_queue_unscreened', registration_url: run.registration_url,
  ...
  queue_builder_amendment: 'v0.3.2-missing-title-preservation',
  raw_rows_missing_title: rawRowsMissingTitle,
  screening_records_missing_title: queue.filter((item) => !item.title).length,
