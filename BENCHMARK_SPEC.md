...
A public benchmark release requires source hashes, license/redistribution checks, a double-coded adjudication sample, campaign-level split integrity, false-positive/abstention results, and a machine-readable card describing bias and prohibited uses. The current source-by-source rights review permits the repository's conservative use: external articles remain link-and-hash references, and local reports do not relicense underlying data. Run `node --experimental-strip-types audit_benchmark.ts` to reconcile the cases, rights review, and provisional card and write the machine-readable audit. No account-level accusation is released on a proxy-only case.
