...
A public benchmark release requires source hashes, license/redistribution checks, a double-coded adjudication sample, campaign-level split integrity, false-positive/abstention results, and a machine-readable card describing bias and prohibited uses. No account-level accusation is released on a proxy-only case.
